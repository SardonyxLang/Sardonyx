...
module SDX
  module Datatypes
      ...
      def initialize
        ...
        @fields = {
          ...
          "__mod" => SDXNativeFn.new(Proc(Array(Value), Value?).new do |args|
            ...
            when ArithmeticValue
              val = args[0].as self
              return self.class.new @internal % val.internal, @scope
            else
        ...
        @fields.merge!({
          ...
          "__pow" => SDXNativeFn.new(Proc(Array(Value), Value?).new do |args|
            ...
            when ArithmeticValue
              ...
              case val.internal
              when Int
                return SDXInt.new @internal ** val.internal.as(Int32), @scope
              when BigDecimal
                return SDXInt.new @internal ** val.internal.as(BigDecimal).to_i, @scope
              end
            else
    ...
    class SDXList < Value
      ...
      def initialize(internal : Array(Value), scope = {} of String => Value, name = "anonymous", current = nil, pos = 0)
        @internal = internal
        @scope = scope
        @name = name
        @pos = pos
        @current = current || @internal[@pos]?
        @fields = {
          "__eq" => SDXNativeFn.new(Proc(Array(Value), Value?).new do |args|
            return SDXBool.new @internal == args[0].internal, @scope
          end, 1, scope),
